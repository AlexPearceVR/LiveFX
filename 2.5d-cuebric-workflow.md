...
## Working with .2p5d Files

Live FX can import .2p5d files and set them up automatically.&#x20;

### Greenscreen / non-projection mapping
...
The two main things we will need to adjust throughout this process is the Trans Z and the Width/Height (linked).&#x20;
...
For the Sky and the Water layers or layers that are hundreds of miles away, we want these to **not move** when we move the camera in x,y space, so what we should do is **Unlink from Camera** from the **Projection Tab** in the **Plane->Wall menu.** Make sure th**e Link to Shot Camera is off.**
